...
<option value="<?php echo $v;?>" <?php if($bank == $v) { ?>selected<?php } ?>
><?php echo $v;?></option>;
<?php } } ?>
</select>
&nbsp;
...
&nbsp;
<input type="submit" value=" 搜 索 " class="btns"/>&nbsp;
<input type="button" value=" 重 置 " class="btns" onclick="Go('?action=<?php echo $action;?>');"/>
</div>
</form>
<div class="bd">
...
<td<?php if($v['note']) { ?> title="原因及备注:<?php echo $v['note'];?>"<?php } ?>
><?php echo $v['dstatus'];?></td>
</tr>
<?php } } ?>
<tr align="center">
...
<tr>
<td class="tl">收款户名</td>
<td class="tr"><?php if($member['banktype']) { ?><?php echo $member['company'];?><?php } else { ?><?php echo $member['truename'];?><?php } ?>
</td>
</tr>
<tr>
<td class="tl">帐号类型</td>
<td class="tr"><?php if($member['banktype']) { ?>对公<?php } else { ?>对私<?php } ?>
</td>
</tr>
<tr>
<td class="tl">收款帐号</td>
...
<td class="tr">
<input type="radio" name="banktype" value="0"<?php if($member['banktype']==0) { ?> checked<?php } ?>
 onclick="Dd('name').innerHTML='<?php echo $member['truename'];?>';"/> 对私
<input type="radio" name="banktype" value="1"<?php if($member['banktype']==1) { ?> checked<?php } ?>
 onclick="Dd('name').innerHTML='<?php echo $member['company'];?>';"/> 对公
</td>
</tr>
<tr>
